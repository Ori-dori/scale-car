...
class Controller():
    ...
    def __init__(self):
        ...
        self.speed_lane = 1.0 # default speed
        self.speed_rabacon = 0.7 # rabacon speed
...
# ========== preprocessing via HSV ==========================
    ...
    def follow_lane(self): # 차선데이터를 받아서 차선을 따라서 주행하도록 하는 함수
        ...
        # obstacle
            ...
            #         if abs(statistics.mean(self.y_list[5:15]) - statistics.mean(self.y_list[-15:-5])) >= 0.4 :
            #             self.dynamic_flag = 1
            #             self.y_list.clear
            #         else:
            #             self.static_flag = 1
            ...
            # static obstacle
            # competition day
            if self.obstacle_cnt ==  0 or self.obstacle_cnt == 1 or self.obstacle_cnt ==3 or self.obstacle_cnt ==4:
                ...
                if self.current_lane == "RIGHT":
                    ...
                    while t2-self.turn_left_t1 <= 1:
                        ...
                        t2 = rospy.get_time()
                    while t2- self.turn_left_t1 <= 1.2 :
                        self.change_line_right()
                        t2 = rospy.get_time()
                    self.current_lane = "LEFT"
                    self.static_flag = 0
                    self.turnrospy.loginfo("STATIC OBSTACLE")
                self.obstacle_cnt += 1
                # if the car is driving depending on "right" window
                if self.current_lane == "RIGHT":
                    rospy.logwarn("IN RIGHT")
                ...
                # if the car is driving depending on "left" window
                    ...
                    while t2-self.turn_right_t1 <= 1:
                        self.change_line_right()
                        t2 = rospy.get_time()
                    while t2-self.turn_right_t1 <= 1.2 :
                        self.change_line_left()
                        t2 = rospy.get_time()
                    self.current_lane = "RIGHT"
                    self.static_flag = 0
