...
class Controller():
    def __init__(self):
        ...
        # rospy.Subscriber("warning_cnt", Int32,self.warncnt_callback)
        rospy.Subscriber("rabacon_drive", Float32, self.rabacon_callback)
        rospy.Subscriber("obstacles", Obstacles, self.rabacon_`callback)
     

    def warncnt_callback(self, _data):
...
#        18:30 - 20/30/20
        ...
        if self.initialized == False:
            ...
            cv2.createTrackbar('low_H', 'Simulator_Image', 65, 255, nothing)
            cv2.createTrackbar('low_S', 'Simulator_Image', 45, 255, nothing)
            cv2.createTrackbar('low_V', 'Simulator_Image', 20, 255, nothing)
    ...
    def follow_lane(self, flag = False): # 차선데이터를 받아서 차선을 따라서 주행하도록 하는 함수
        ...
        if self.slow_down_flag == 1:
            ...
            publishing_data.header.frame_id = "base_link"
            publishing_data.drive.steering_angle = self.error_lane * 0.003 # error 정도에 따라서 조향 
            publishing_data.drive.speed = 0.8
            self.drive_pub.publish(publishing_data) # 실제 publish 하는 부분 
