...
class SlideWindow:
    ...
    def slidewindow(self, img):
        ...
        # 337 -> 310
        good_left_inds = ((nonzerox >= width/2 - 190) & (nonzeroy >= nonzerox * 0.33 + 300) & (nonzerox <= width/2 - 70)).nonzero()[0]
        good_right_inds = ((nonzerox >= width/2 + 0) & (nonzeroy >= nonzerox * (-0.48) + 500) & (nonzerox <= width/2 + 140)).nonzero()[0]
        ...
        if line_flag != 3:
            ...
            for window in range(0, nwindows):
                if line_flag == 1: 
                    ...
                    cv2.rectangle(out_img, (win_x_low + int(width * 0.35), win_y_low), (win_x_high + int(width * 0.35), win_y_high), (255, 0, 0), 1)
